...
_possible_args = OrderedDict({
    ...
    'method': [None, 'multi', ], ### TODO Implement a callable insert method?
})
...
params = {
    k: list(itertools.product(
        *[_v for _k, _v in _possible_args.items()
            if _k in args[k]]
    )) for k in args
}
params_strings = {
    k: (
        ','.join([str(_k) for _k in args[k]])
    ) for k in args
}
